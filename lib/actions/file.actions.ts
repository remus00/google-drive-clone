...
const uploadFile = async ({ file, ownerId, accountId, path }: UploadFileProps) => {
...
const getFiles = async ({ types = [] }: GetFilesProps) => {
...
const updateFileUsers = async ({ fileId, emails, path }: UpdateFileUsersProps) => {
    const { databases } = await createAdminClient();
...
export { deleteFile, getFiles, renameFile, updateFileUsers, uploadFile };
